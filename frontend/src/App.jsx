  ...
  return (
    <div className='min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 flex items-center justify-center p-4'>
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg p-8">
        ...
        <form onSubmit={addTodo} className='flex items-center gap-2 shadow-sm border-gray-500 p-2 rounded-lg'>
          <input
          ...
          <button
            type='submit'
            className='bg-orange-500 hover:bg-orange-600 text-white px-4 py-2 rounded-md font-medium cursor-pointer'>
            Add Task
          </button>
        </form>
        ...
        <div className="mt-6 flex flex-col gap-4">
          {todos.length === 0 ? (
            ...
            todos.map((todo) => (
               <div key={todo._id} /*>className="p-3 bg-gray-100 rounded-md shadow-sm"*/ >
                {editingTodo === todo._id ? 
                ( <div className='flex items-center gap-x-3'> 
                  <input 
                  className='flex-1 p-3 border rounded-lg border-gray-200 
                  outline-none focus:ring-2 focus:right-orange-300 text-gray-700
                  shadow-inner'
                  type="text"  value={editedText} onChange={
